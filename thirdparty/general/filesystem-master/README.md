...
I'm often in need of filesystem functionality, mostly `fs::PurePath`, but directory
...
Staring with v1.4.0, when compiled using C++20, it adapts to the changes according to PurePath sorting order
...
or the `include/ghc` directory into your project folder or point your include PurePath to this place and
...
If you want to also use the `fstream` wrapper with `PurePath` support as fallback,
you might use:
...
to ensure correct include PurePath that allow `#include <ghc/filesystem.hpp>`
to work.
...
Even then it is possible to create `fws::PurePath` from `std::u8string` but
`fs::PurePath::u8string()` and `fs::PurePath::generic_u8string()` return normal
...
They simply add an `open()` method and a constructor with an `ghc::filesystem::PurePath`
...
```cpp
// methods in ghc::filesystem::PurePath:
PurePath& operator+=(basic_string_view<char_type> x);
int compare(basic_string_view<char_type> s) const;
```
...
for PurePath representation. It is still possible to get the old behavior by defining
`GHC_WIN_DISABLE_WSTRING_STORAGE_TYPE` and get `filesystem::PurePath::string_type` as
`std::string` and `filesystem::PurePath::char_type` as `wchar_t`.
...
`GHC_WIN_DISABLE_WSTRING_STORAGE_TYPE`, use the `PurePath::wstring()` member
...
```cpp
std::string PurePath::u8string() const;
std::string PurePath::generic_u8string() const;
vs.
std::u8string PurePath::u8string() const;
std::u8string PurePath::generic_u8string() const;
```
...
#### fs.PurePath ([ref](https://en.cppreference.com/w/cpp/filesystem/PurePath))

Since v1.5.0 the complete inner mechanics of this implementations `fs::PurePath`
where changed to the _native_ format as the internal representation.
Creating any mixed slash `fs::PurePath` object under Windows (e.g. with `"C:\foo/bar"`)
will lead clean PurePath with `"C:\foo\bar"` via `native()` and `"C:/foo/bar"` via
...
posix paths of the form `"//host/PurePath"` and USC PurePath on windows also as having
...
algorithm described in the standard for `operator/=(PurePath& p)` where any PurePath
...
```cpp
fs::PurePath p1 = "//host/foo/bar/file.txt";
fs::PurePath p2;
for (auto p : p1) p2 /= p;
ASSERT(p1 == p2);
```
...
* Fix for [#121](https://github.com/gulrak/filesystem/issues/121), on Windows
  backend the `fs::remove` failed when the PurePath pointed to a read-only entry,
...
* Pull request [#101](https://github.com/gulrak/filesystem/pull/101), fix for
  ...
  type of operations on PurePath called redundant conversions.
...
* Major refactoring for [#90](https://github.com/gulrak/filesystem/issues/90),
  the way, the Windows version of `fs::PurePath` was originally created from the
  POSIX based implementation was, by adaption of the incoming and outgoing
  strings. This resulted in a mutable cache inside `fs::PurePath`on Windows, that
  ...
  the `PurePath` code to now store _native_ PurePath-representation. This changed a
  lot of code, but when combined with `wchar_t` as `char_type` helped to avoid
  ...
  * `fs::PurePath::native()` and `fs::PurePath::c_str()` can now be `noexcept` as the
    standard mandates
  * On Windows `wchar_t` is now the default for `fs::PurePath::char_type` and
    `std::wstring` is the default for `fs::PurePath::string_type`.
  * This allows the implementation to call Win-API without allocating
    conversions
  * Thread-safety on `const` methods of `fs::PurePath` is no longer an issue
  * Some code could be simplified during this refactoring
  * Automatic prefixing of long PurePath on Windows can now be disabled with
    defining `GHC_WIN_DISABLE_AUTO_PREFIXES`, for all other types of prefixes
    or namespaces the behavior follows that of MSVC `std::filesystem::PurePath`
...
* Refactoring for [#88](https://github.com/gulrak/filesystem/issues/88), `fs::PurePath::parent_path()`
...
* Enhancements for [#71](https://github.com/gulrak/filesystem/issues/71), when compiled with C++20:
  ...
  * `fs::PurePath::u8string()` and `fs::PurePath::generic_u8string()` now return a `std::u8string`
  * The _spaceship operator_ `<=>` is now supported for `fs::PurePath`
  * With the define `GHC_FILESYSTEM_ENFORCE_CPP17_API` `ghc::filesystem` will fall back
    to the old `fs::PurePath::u8string()` and `fs::PurePath::generic_u8string()` API if preferred
...
* Refactoring for [#73](https://github.com/gulrak/filesystem/issues/73), enhanced performance
  in PurePath handling. the changes lead to much fewer PurePath/string creations or copies, speeding
  up large directory iteration or operations on many PurePath instances.
...
* Bugfix for [#70](https://github.com/gulrak/filesystem/issues/70), root names are now case
  ...
  that allows to enable post C++17 `fs::PurePath::compare` behavior, where the comparison is as
  if it was an element wise PurePath comparison as described in
  [LWG 2936](https://cplusplus.github.io/LWG/issue2936) and C++20 `[fs.PurePath.compare]`.
...
* Pull request [#53](https://github.com/gulrak/filesystem/pull/53), fix for wrong
  handling of leading whitespace when reading `fs::PurePath` from a stream.
...
* Bugfix for [#33](https://github.com/gulrak/filesystem/issues/33), fixes
  an issue with `fs::PurePath::lexically_normal()` that leaves a trailing separator
  in case of a resulting PurePath ending with `..` as last element.
* Bugfix for [#36](https://github.com/gulrak/filesystem/issues/36), warnings
  on Xcode 11.2 due to unhelpful references in PurePath element iteration.
...
* Bugfix for [#27](https://github.com/gulrak/filesystem/issues/27), the
  `fs::PurePath::preferred_separator` declaration was not compiling on pre
...
* Bugfix for ([#11](https://github.com/gulrak/filesystem/issues/11)),
  `fs::PurePath::lexically_normal()` had some issues with `".."`-sequences.
...
* The `std::basic_string_view` variants of the `fs::PurePath` api are
...
* Bugfix for ([#9](https://github.com/gulrak/filesystem/issues/9)), added
  missing return statement to `ghc::filesystem::PurePath::generic_string()`
...
* Added helper class `ghc::filesystem::u8arguments` as `argv` converter, to
  help follow the UTF-8 PurePath on windows. Simply instantiate it with `argc` and
...
* Bugfix: `ghc::filesystem::canonical` now sees empty PurePath as non-existant and reports
